...
function setupRoutes() {
  ...
  function getActiveAdvertiserAds(db: any, userIp?: string) {
    return {
      offerWallAds: [],
      extraOfferWallAd: null,
      extraSponsoredPopupAd: null,
      activeBanners: {}
    };
  }

  // Record impression / view stub (safe no-op)
  ...
  app.get("/api/links/resolve/:code", (req, res) => {
    ...
    res.json({ 
      ...
      settings: {
        ...
        enableClickAdGate: db.settings.enableClickAdGate ?? (db.settings.enableNeonAdGate ?? false),
        clickAdRandomRotation: db.settings.clickAdRandomRotation ?? true,
        clickAdCodes: db.settings.clickAdCodes || [],
        clickAd728x90: db.settings.clickAd728x90 || "",
        clickAd300x250: db.settings.clickAd300x250 || "",
        clickAd300x600: db.settings.clickAd300x600 || "",
        clickAd468x60: db.settings.clickAd468x60 || "",
        clickAd320x50: db.settings.clickAd320x50 || "",
        clickAdCustom1: db.settings.clickAdCustom1 || "",
        clickAdCustom2: db.settings.clickAdCustom2 || "",
